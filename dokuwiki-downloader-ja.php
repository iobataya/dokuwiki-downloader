 ...
 * 
 * DokuWikiをWebブラウザから自動的にインストールスクリプトです。
 * オリジナルのダウンローダのインターフェースを日本語にし、
 * local.protected.phpに優先設定を加えています。(1500行以降)
 */

$TAG = 'stable'; // the DokuWiki version to be downloaded

/// パーミッションを0755, 0644に設定する。
/// この設定はlocal.protected.php内にも追加する。
$DPERM   = 0755;
$FPERM   = 0644;
...
function step1(){
    ...
    if(!isset($_GET['go'])){
        say("<p>このスクリプトは<strong>DokuWiki $TAG</strong> をダウンロードし、
                あなたのWebサーバーの<strong>$INSTALL_DIR</strong>にインストールします。
                もしこのスクリプトがインストールしたいフォルダ内にない場合は、
                dokuwikiのルートフォルダにしたい場所に移動させて下さい。</p>
             <form method=\"get\" action=\"\">
                 <input type=\"hidden\" name=\"go\" value=\"1\" />
                 <input type=\"submit\" value=\"ダウンロードを開始する。\" />
             </form>
            ");
        html_footer();
    }
    ...
    if(!is_writable($INSTALL_DIR)){
        say("<strong>$INSTALL_DIR</strong> は書き込みできないようです。このフォルダに書き込み権限を与えて下さい。");
        html_footer();
    }
}
...
function step2(){
    ...
    say('<a href="'.$DOWNLOAD.'">DokuWikiアーカイブ</a>をダウンロード中...');
    @set_time_limit(120);
    @ignore_user_abort();
    ...
    if(!$data){
        say($http->error);
        say("ダウンロードが失敗しました。.tgzファイル($TGZ)を直接このフォルダにアップロードしてみてください。");
        html_footer();
    }
    ...
    $fp = @fopen($TGZ,"w");
    if(!$fp){
        say("保存に失敗しました。.tgzファイル($TGZ)を直接このフォルダにアップロードしてみてください。");
        html_footer();
    }
    fwrite($fp,$data);
    fclose($fp);
    ...
    say("<p>ダウンロードが正常に終了しました。次のステップに進んで下さい。</p>
         <form method=\"get\" action=\"\">
             <input type=\"hidden\" name=\"go\" value=\"3\" />
             <input type=\"submit\" value=\"アーカイブの展開に進む。\" />
         </form>");
    html_footer();
}
...
function step3(){
    ...
    say("<p>展開は正常に終了しました。次のステップに進んで下さい。</p>
         <form method=\"get\" action=\"\">
             <input type=\"hidden\" name=\"go\" value=\"4\" />
             <input type=\"submit\" value=\"最後のステップに進む。\" />
         </form>");
    html_footer();
}
...
function step4(){
    ...
    global $DPERM;
    /*
     * local.protected.phpに保護される設定を書く。
     */
    $dp = decoct($DPERM);
    $fp = decoct($FPERM);
    $local_protected = fopen("$INSTALL_DIR/conf/local.protected.php","w");
    fwrite($local_protected, "<?php\n");
    fwrite($local_protected, "\$conf['dmode'] = 0$dp;\n");
    fwrite($local_protected, "\$conf['fmode'] = 0$fp;\n");
    fwrite($local_protected, "\$conf['fnencode'] = 'utf-8';\n");
    fwrite($local_protected, "\$conf['lang'] = 'ja';\n");
    fwrite($local_protected, "\$conf['useslash'] = 'useslash';\n");
    fclose($local_protected);
    
    if(@unlink($TGZ)){
        say("DokuWikiアーカイブファイルは削除されました。");
    }else{
        say("DokuWikiのtgzアーカイブを削除できませんでした。後から直接削除して下さい。");
    }
    
    if(@unlink(__FILE__)){
        say("DokuWikiダウンローダスクリプトは削除されました。");
    }else{
        say("このダウンローダスクリプトは削除できませんでした。後から直接削除して下さい。");
    }
    say("<p>これで終わりです。DokuWikiは正しくダウンロードされ、サーバー上に展開されました。
            次のステップで、通常のインストーラが起動して初期設定を行います。
            いくつかの設定はlocal.protected.phpに予め優先設定されています。必要があれば修正して下さい。
            </p>
        <form method=\"get\" action=\"install.php\">
            <input type=\"hidden\" name=\"l\" value=\"ja\" />
             <input type=\"submit\" value=\"初期設定へ移ります。\" />
         </form>
        ");
    html_footer();
}
...
// initialize
$DOWNLOAD    = 'https://github.com/splitbrain/dokuwiki/tarball/'.$TAG;
